...
**Provenance** (generated by `DATABASE_URL=... npx vitest run`)
```
...
density 112 (> 20) → classification RED_OCEAN. **If the hash algorithm drifts, this returns density 0 (WHITESPACE) and fails loudly.**
...
```
operators.ts below 90% due to untested error-handling branches in swap/validate
(unreachable in normal operation — ontology cache miss, DB failure paths).
